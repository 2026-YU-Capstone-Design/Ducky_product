...
import { getKakaoRedirectUri, KAKAO_OAUTH_STATE_KEY } from "@/lib/auth/kakao";
...
function KakaoCallbackContent() {
  ...
  const [message, setMessage] = useState("카카오 로그인을 처리하는 중입니다.");
  ...
  useEffect(() => {
    ...
    async function completeLogin() {
      ...
      if (savedState && state !== savedState) {
        setFailed(true);
        setMessage("카카오 로그인 요청 정보가 일치하지 않습니다.");
        return;
      }
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-[#FAF8F5] px-6 text-center">
      <div className="flex max-w-sm flex-col items-center gap-4">
        {!failed && (
          <Loader2 className="size-8 animate-spin text-[#B88700]" aria-hidden="true" />
        )}
        ...
        {failed && (
          <button
            className="mt-2 rounded-lg bg-[#FECA43] px-4 py-2 text-sm font-bold text-[#2E2A22]"
