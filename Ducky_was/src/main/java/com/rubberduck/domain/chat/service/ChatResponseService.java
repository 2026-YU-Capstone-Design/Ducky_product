...
public class ChatResponseService {
    ...
    String buildHintPrompt(
        ...
        return """
                ...
                - 정답 전체, 개념 정의, 원인 단정, 해결책 제시는 하지 않는다.
                - 사용자가 스스로 확인할 수 있도록 관찰·실험을 유도하는 질문 1개 또는 그에 준하는 짧은 단서만 준다.
                - 1~2문장으로 끝낸다.
    ...
    private String buildTurnPrompt(
        ...
        String modeInstruction = mode == ReplyMode.DIRECT_ANSWER
                ...
                - 정답 설명, 개념 정의, 해결책 제시, 원인 단정은 하지 마.
                - 사용자가 직접 말하게 만드는 질문 1개로 응답을 끝낸다.
                - 사용자가 이미 말한 내용을 짧게 되짚은 뒤, 스스로 생각을 이어가게 할 구체적인 질문을 던져.
        ...
        return """
                ...
                - 러버덕 방식: 정답 설명·원인 단정 금지, 질문으로 끌고 가서 사용자가 말하게 만든다.
                - 한 응답에 질문은 1개만 넣고, 러버덕 모드 응답은 질문으로 끝낸다.
                - 사용자가 올린 문서 맥락은 관련 있을 때만 사용한다.
                - "이름", "그거", "다른 것", "이건" 같은 짧은 후속 질문은 현재 세션 전체 대화 기록에서 지시 대상을 찾되, 정답 대신 그 대상에 대해 사용자가 스스로 설명하도록 유도하는 질문을 던진다.
    ...
    private String processingInstruction(String processing) {
        return switch (processing) {
            case "reflective" -> "조건, 가정, 원인을 사용자에게 직접 말하게 유도하는 질문을 던진다. 원인을 대신 단정하지 않는다.";
            case "active" -> "바로 확인할 작은 실험·관찰을 질문 형태로 제안한다. 방법을 대신 설명하지 않는다.";
    ...
    private String expressionInstruction(String expression) {
        return switch (expression) {
            case "verbal" -> "자신의 말로 풀어 설명하게 만드는 질문을 던진다. 핵심 문장을 대신 써주지 않는다.";
            case "visual" -> "구조, 흐름, 비교를 스스로 그리거나 묘사하게 유도하는 질문을 던진다.";
            default -> "구조와 말로 정리하게 사용자가 직접 말하도록 질문으로 이끈다.";
        };
    }

    private String understandingInstruction(String understanding) {
        return switch (understanding) {
            case "global" -> "전체 그림부터 스스로 잡게 한 뒤 세부로 내려가게 질문으로 이끈다.";
